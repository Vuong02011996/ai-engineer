...
NvidiaDeepStreamDevelopmentLicense.pdf  bin          lib            samples`
+ deepstream-6.4: 
`LICENSE.txt                             README       entrypoint.sh  reference_graphs                    sources       update_rtpmanager.sh                    version
LicenseAgreement.pdf                    README.rhel  install.sh     rtpjitterbuffer_eos_handling.patch  tools         user_additional_install.sh
...
+ samples: 
`configs  models  prepare_classification_test_video.sh  prepare_ds_triton_model_repo.sh  prepare_ds_triton_tao_model_repo.sh  streams  triton_model_repo  triton_tao_model_repo  trtis_model_repo`
  + configs:
    `deepstream-app  deepstream-app-triton  deepstream-app-triton-grpc  tao_pretrained_models`
    + deepstream-app:
    ...
    + tao_pretrained_models: Run Test with TAO MODEL IN HERE: `deepstream-app -c deepstream_app_source1_peoplenet.txt`
      + `README.md                                                            deepstream_app_source1_peoplenet.txt      efficientdet_d0_labels.txt  mrcnn_labels.txt          ssd_labels.txt
        ...
        `
  + models:
    + `Primary_Detector  SONYC_Audio_Classifier  Secondary_VehicleMake  Secondary_VehicleTypes  Segmentation  tao_pretrained_models`
...
+ sources: **CLONE Deepstream python apps in HERE** `deepstream_python_apps`
`SONYCAudioClassifier  TritonBackendEnsemble  TritonOnnxYolo  apps  deepstream_python_apps  gst-plugins  includes  libs  objectDetector_Yolo  tools  tracker_ReID
`
