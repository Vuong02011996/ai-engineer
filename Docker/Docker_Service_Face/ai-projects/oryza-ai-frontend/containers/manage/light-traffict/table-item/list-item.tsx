...
export function ListItemTable(props: IListItemTableProps) {
  ...
  function getColor(signal: string) {
    let key = signal.toLowerCase();
    return colorMap[key] || "Không xác định";
  }
  ...
  return (
    <div className="w-full flex flex-row min-w-[1200px] table-row-custom items-center border-b-2 border-[#F8F8F8]">
      ...
      <div className="w-[25%] py-6 pr-3 flex flex-col justify-start">
      ...
      <div className="w-[30%] py-6 pr-3 flex justify-start">
        {_renderText(getColor(data?.signal || ""))}
      </div>

      <div className="w-[25%] py-6 pr-3 flex justify-start">
        {data.timestamp
