...
export function ListItemTable(props: IListItemTableProps) {
  ...
  return (
    <div className="w-full flex flex-row min-w-[1200px] table-row-custom items-center border-b-2 border-[#F8F8F8]">
      ...
      <div className="w-[15%] py-6 flex justify-center">
        {_renderText(data?.total_people_detected)}
      </div>
      <div className="w-[15%] py-6 flex justify-center">
        {_renderText(data?.crowd_members_count)}
      </div>
      <div className="w-[15%] py-6 flex justify-center">
        {_renderText(data?.crowd_alert_threshold)}
      </div>
