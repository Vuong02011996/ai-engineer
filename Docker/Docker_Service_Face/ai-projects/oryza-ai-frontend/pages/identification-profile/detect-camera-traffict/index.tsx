...
import { CameraTraffictTable } from "@/containers/identification-profile/camera-traffict/table-item";
import tabData from "@/data/identification-profile";
import {
  optionFilterCrow,
} from "@/data/identification-profile/crowd";
import headData from "@/data/identification-profile/camera-traffict";
...
export interface IDetectItemsForgottenProps {}

export default function DetectItemsForgotten(
  props: IDetectItemsForgottenProps
) {
  const { profile } = useAuth();
  const [loading, setloading] = useState(false);
  const [data, setData] = useState<CameraTraffictDetection[]>([]);
  ...
  const hanldeGetAll = async () => {
    ...
    try {
      ...
      const { data } = await cameraTraffictApi.getAllInfo(params);
  ...
  const getCount = async () => {
    try {
      ...
      let { data } = await cameraTraffictApi.getCount(params);
  ...
  useEffect(() => {
    ...
    if (debounce) {
      setPage("/identification-profile/detect-camera-traffict", 1);
    }
  ...
  return profile?.is_admin ? (
    <section className="p-[24px] h-full">
      <SeoPage title="Nhận diện tín hiệu giao thông" />
        ...
        <OpactityAnimation className={"h-calc112"}>
          <div className="flex-1 h-full pt-2">
            ...
            <PaginationTable
              ...
              maxPage={maxPage}
              basePath={"/identification-profile/detect-camera-traffict"}
            />
          </div>
        </OpactityAnimation>
      </div>
...
DetectItemsForgotten.Layout = HomeLayout;
