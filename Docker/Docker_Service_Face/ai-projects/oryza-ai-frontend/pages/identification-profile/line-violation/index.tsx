...
import { laneViolationHead } from "@/data/identification-profile/lane-violation";
...
export interface ILaneViolationProps {}

export default function LaneViolation(props: ILaneViolationProps) {
  ...
  useEffect(() => {
    ...
    if (debounce) {
      setPage("/identification-profile/lane-violation", 1);
    }
  ...
  return profile?.is_admin ? (
    <section className="p-[24px] h-full">
      <SeoPage title="Nhận diện hành vi lấn vạch" />
        ...
        <OpactityAnimation className={"h-calc112"}>
          <div className="flex-1 h-full pt-2">
            ...
            <PaginationTable
              ...
              maxPage={maxPage}
              basePath={"/identification-profile/line-violation"}
            />
          </div>
        </OpactityAnimation>
      </div>
...
LaneViolation.Layout = HomeLayout;
