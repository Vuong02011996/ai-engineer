...
from app.services.video_offline_face_detection.face_detection_model_video_offline import run_roll_call


blueprint = Blueprint("Video_Offline_Face_Service", __name__)
# Using register_blueprint
api = Api(blueprint, title="Video_Offline_Face_Service", version="1.0.0", description="Swagger API Homepage")
...
#
@api.route("/analyze_video/estimate_time", methods=["POST"])
class StartById(Resource):
    def post(self):
        ...
        video_length = request_data.get("video_length")
        width = request_data.get("width")
        height = request_data.get("height")
        type = request_data.get("type")
        skip_frames = request_data.get("skip_frames")
        return {"data": 5}
...
class StartById(Resource):
    def post(self):
        ...
        {
            "type": "face",
            "video_url": "/home/oryza/Videos/video_test_offline.mp4",
            "black_list": ["https://s3.oryza.vn/face/attendance_test/test_106_track_111723531645.jpg"],
            "image_extension":  "jpg"
        }
        Returns:
        ...
        black_list = request_data.get("black_list")
        type = request_data.get("type")
        ...
        # Create new process
        process_name = id_cam_mg + "_roll_call_" + str(len(process_dal.find_all_item())) + "_" + \
        ...
        def process_data():
            ...
            "In function khong duoc truyen bien outside its scope "
