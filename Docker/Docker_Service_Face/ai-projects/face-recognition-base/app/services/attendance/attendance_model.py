...
class InfoCam(object):
    def __init__(self, cam_name, process_name, test_video):
...
def run_roll_call(item_cam, process_name, end_time_s, test_video=False, cv2_show=True, window="test"):
    ...
    cam = InfoCam(input_path, process_name, test_video)
    # -------------------------------------------------------------------------
    ...
    while cam.cap.isOpened():
        ...
        if test_video is False:
            print("frame_count_" + item_cam["name_cam"] + ":    ", frame_count)
...
if __name__ == '__main__':
    ...
    process_stream = None
    process_name = "660125d24586785d46f8808c_roll_call_0_2024:03:25-14:55:04"
    thread_safe_region_manager = []
    time_now = datetime.now()
    time_now_seconds = time_now.timestamp()
    end_time_s = time_now_seconds + 5000
    run_roll_call(input_path, process_name, end_time_s, test_video=True, cv2_show=True)
