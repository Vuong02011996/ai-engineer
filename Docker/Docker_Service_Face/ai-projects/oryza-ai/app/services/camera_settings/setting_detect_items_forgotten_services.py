...
from app.models.camera_settings.setting_detect_items_forgotten_model import SettingDetectItemsForgotten

from app.schemas.camera_settings.setting_detect_items_forgotten_schemas import (
    SettingDetectItemsForgottenCreate as SDIFCreate,
    SettingDetectItemsForgottenUpdate as SDIFUpdate,
)
...
class CRUDSettingDetectItemsForgottenServices(CRUDBase[SettingDetectItemsForgotten, SDIFCreate, SDIFUpdate]):

    def create_setting(self, res: SDIFCreate):
    ...
    def get_by_id_camera(self, id_camera: str):
        return self.engine.find_one(
            SettingDetectItemsForgotten, SettingDetectItemsForgotten.camera == ObjectId(id_camera)
        )

    def get_all(self, id_company: str):
        return list(
            self.engine.find(
                SettingDetectItemsForgotten, SettingDetectItemsForgotten.id_company == id_company
            )
        )
    ...
    def get_all_info(self, user: User, page: int = 0, page_break: bool = False, data_search=None,
        ...
        return get_all_info(user=user, page=page, page_break=page_break, data_search=data_search, filter=filter,
                            name_table="setting_detect_items_forgotten", engine=self.engine)

    def get_count_info(self, user: User, data_search=None, filter: FilterSettingCrow = FilterSettingCrow.ALL):
        return get_count_info(user=user, data_search=data_search, filter=filter, name_table="setting_detect_items_forgotten",
                              engine=self.engine)

setting_detect_items_forgotten_services = CRUDSettingDetectItemsForgottenServices(SettingDetectItemsForgotten)
