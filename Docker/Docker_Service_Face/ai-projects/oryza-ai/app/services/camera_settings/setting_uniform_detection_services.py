...
from app.models.camera_settings.setting_uniforms_detection import SettingUniformsDetection
from app.schemas.camera_settings.setting_uniforms_schemas import SettingUniformsCreate, SettingUniformsUpdate
...
class CRUDSettingUniform(
    CRUDBase[
        SettingUniformsDetection, SettingUniformsCreate, SettingUniformsUpdate
    ]
):
    def create_setting(self, res: SettingUniformsCreate):
    ...
    def update_setting(
            self, res: SettingUniformsUpdate, id: str
    ):
    ...
    def get_by_id_camera(self, id_camera: str):
        return self.engine.find_one(
            SettingUniformsDetection, SettingUniformsDetection.camera == ObjectId(id_camera)
        )

    def get_all(self, id_company: str):
        return list(
            self.engine.find(
                SettingUniformsDetection, SettingUniformsDetection.id_company == id_company
            )
        )
    ...
    def get_all_info(self, user: User, page: int = 0, page_break: bool = False, data_search=None,
        ...
        return get_all_info(user=user, page=page, page_break=page_break, data_search=data_search, filter=filter,
                            name_table="setting_uniforms_detection", engine=self.engine)

    def get_count_info(self, user: User, data_search=None, filter: FilterSettingCrow = FilterSettingCrow.ALL):
        return get_count_info(user=user, data_search=data_search, filter=filter, name_table="setting_uniforms_detection",
                              engine=self.engine)


setting_uniform_services = CRUDSettingUniform(SettingUniformsDetection)
