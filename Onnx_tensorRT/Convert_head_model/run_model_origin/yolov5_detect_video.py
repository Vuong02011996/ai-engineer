...
from Onnx_tensorRT.Convert_head_model.run_model_origin.my_utils import draw_det_when_track
import time
...
def inference(cam, frame_detect_queue, detections_queue): #, tracking_queue):
    while cam.cap.isOpened():
        ...
        boxes, labels, scores = y5_model.predict(image_rgb, label_select=["head"])
        print("boxes: ", boxes)
